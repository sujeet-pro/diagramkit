...
Color schemes for different platforms and component types in Excalidraw diagrams rendered by diagramkit.
...
diagramkit automatically renders dark mode variants. These palettes are designed to work well with diagramkit's `postProcessDarkSvg()` contrast optimization, which:

- Darkens fill colors with high luminance (>0.4 WCAG)
- Preserves hue so colored nodes keep their visual identity
- Inverts text colors for readability

**Guidelines for color selection:**

- Prefer the palettes listed above -- they have been tested with dark mode
- Avoid very light fills (close to `#ffffff`) -- they lose distinction when darkened
- Avoid neon/fluorescent colors -- they may produce harsh results in dark mode
- The stroke color should always be darker than the fill color
- Use transparent backgrounds for grouping rectangles
...
```
User (ellipse)     -> #e7f5ff / #1971c2
API Gateway        -> #dee2e6 / #495057
Auth Service       -> #ffc9c9 / #e03131
User Service       -> #d0bfff / #7048e8
Order Service      -> #d0bfff / #7048e8
PostgreSQL         -> #b2f2bb / #2f9e44
Redis Cache        -> #ffe8cc / #fd7e14
...
```
CloudFront         -> #ff9900 / #cc7a00
ALB                -> #8c4fff / #6b3dcc
ECS Cluster        -> #ff9900 / #cc7a00
RDS                -> #3b48cc / #2d3899
S3                 -> #3f8624 / #2d6119
SQS                -> #ff9900 / #cc7a00
Lambda             -> #ff9900 / #cc7a00
VPC Group          -> transparent / #8c4fff (dashed)
```

### Data Pipeline

```
Source (Kafka)     -> #fff3bf / #fab005
Transformer        -> #d0bfff / #7048e8
ML Model           -> #e599f7 / #9c36b5
Data Lake (S3)     -> #ffec99 / #f08c00
Database           -> #b2f2bb / #2f9e44
Dashboard          -> #a5d8ff / #1971c2
```
