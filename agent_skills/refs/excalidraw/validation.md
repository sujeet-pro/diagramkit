...
Checklists, validation algorithms, and common bug fixes for Excalidraw JSON diagrams rendered by diagramkit.
...
FUNCTION validateDiagram(elements):
  ...
  // 5. Validate no diamond shapes
  FOR each element IN elements:
    IF element.type == "diamond":
      errors.append("Diamond shape {element.id} found -- use rectangle instead")
...
- [ ] Identified all components from codebase or description
...
- [ ] Arrow `x,y` calculated from shape edge (not center)
...
- [ ] File is valid JSON
- [ ] `source` field is set to `"diagramkit"`
- [ ] `viewBackgroundColor` is `"#ffffff"`
...
### Bug: Text appears at wrong position inside shape

**Cause**: Text x/y not calculated from shape position.

**Fix**: Apply text positioning formulas:
...
### Bug: Arrow bounding box warning

**Cause**: Arrow `width`/`height` properties don't match the actual point extents.

**Fix**: Calculate from points:
...
### Bug: Diagram looks wrong in dark mode
...
Before writing the `.excalidraw` file, mentally verify:

1. Count shapes with labels -- each needs 2 elements (shape + text). Total text elements = total labeled shapes.
2. Count arrows -- each needs `elbowed: true`, `roundness: null`, `roughness: 0`.
3. Count unique IDs -- must equal total elements.
4. Verify arrow start/end positions are at shape edges.
5. Verify all `boundElements` and `containerId` cross-references are consistent.
