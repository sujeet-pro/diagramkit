...
- `.agents/skills/pagesmith-*` — **installer-managed versioned-pointer stubs** for every skill shipped by the installed `@pagesmith/core`, `@pagesmith/site`, and `@pagesmith/docs` packages (this repo's docs site is built with them). Generated by `npx pagesmith skills install`; each stub's body delegates to `node_modules/<pkg>/skills/<name>/SKILL.md`, so guidance always matches the installed version. Never edit stubs by hand — rerun the installer. `npm run validate:skills` (`pagesmith skills install --check`, part of `cicd`) fails when stubs drift from the installed packages.
